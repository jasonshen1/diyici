...
1. 必须直接基于OCR识别出的产品信息（产品名称、成分、营养成分、品牌等）进行分析
2. 不要询问用户"图中是什么产品"，而是直接基于OCR结果给出复刻方案
3. 如果OCR内容是产品标签/包装文字，请解析出：产品名称、功能定位、核心成分、规格、品牌等信息
4. 基于这些信息提供详细的复刻方案（配方、包装、成本、供应链等）

如果用户上传了图片但OCR失败（提示"OCR识别失败"或"识别结果为空"），请明确告知用户无法识别图片内容，并请用户文字描述产品。`;
...
【重要提示 - 图片内容处理】
用户输入中可能包含"【用户上传的产品图片 - 需要复刻/分析】"标记和OCR识别结果。
如果用户要求"复刻图中产品"，请检查【谋局者】和【执行者】是否：
1. 正确解析了OCR中的产品信息（名称、成分、规格等）
2. 基于OCR内容提供了针对性的复刻方案
3. 没有忽略OCR内容而给出无关方案

如果发现他们忽略了OCR内容或给出无关方案，必须判定为FAIL并要求重新分析图片内容。`;
...
【重要提示 - 图片内容处理】
用户输入中可能包含"【用户上传的产品图片 - 需要复刻/分析】"标记和OCR识别结果。
如果本次任务是"复刻图中产品"，在复盘中需要：
1. 确认OCR识别出的产品信息是否被充分利用
2. 总结基于产品图片进行复刻的方法论
3. 万能模板应包含"产品复刻"的标准流程（图片分析→信息提取→方案制定→执行落地）

如果OCR识别失败导致任务偏差，请在复盘中指出并建议改进方案。`;
        default:
