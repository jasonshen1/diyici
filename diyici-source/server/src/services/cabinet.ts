...
export class CabinetService {
  async runCabinet(userInput: string, ocrResult?: string): Promise<number> {
    try {
  ...
  async executeFourStepProcess(taskId: number, ocrResult?: string): Promise<void> {
    try {
      ...
      const planningResult = await this.callAIWithFallback(ROLES.PLANNER, task.user_input, ocrResult);
      ...
      do {
        ...
        const executorInput = retryCount > 0 && reviewResult 
          ? `根据以下修改建议重新执行：\n${reviewResult}\n\n原始计划：${planningResult}`
          : planningResult;
        executionResult = await this.callAIWithFallback(ROLES.EXECUTOR, executorInput, ocrResult);
        ...
        reviewResult = await this.callAIWithFallback(ROLES.REVIEWER, executionResult, ocrResult);
      ...
      const finalResult = await this.callAIWithFallback(ROLES.FINALIZER, finalizerInput, ocrResult);
