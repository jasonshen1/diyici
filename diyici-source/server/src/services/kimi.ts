...
// 调用Kimi API的函数
async function callKimiAPICore(
  role: string,
  content: string,
  context?: string
): Promise<string> {
...
// 主API调用函数，优先使用Kimi，失败后使用DeepSeek
export async function callKimiAPI(
  ...
  try {
    ...
    return await callKimiAPICore(role, content, context);
  } catch (error) {
    console.error('Kimi API调用失败，尝试使用DeepSeek API...');
    try {
      // 失败后使用DeepSeek API
      console.log('正在调用DeepSeek API...');
      return await callDeepSeekAPI(role, content, context);
    } catch (deepSeekError) {
      console.error('DeepSeek API调用也失败:', deepSeekError);
      throw new Error('所有AI API调用失败，请检查API密钥和网络连接');
    }
  }
}
